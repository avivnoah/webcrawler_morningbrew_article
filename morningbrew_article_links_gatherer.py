...
def gather_website_links():
    print(f'Link amount to be extracted: {SCROLL_RANGE * 12}')
    with sync_playwright() as p:
        ...
        for i in range(4):
            try:
                ...
                page.wait_for_selector('div.cqKibH', timeout=TIMEOUT_LIMIT)
                break
            except:
                continue
        ...
        # A list of all the links gathered
        links_list = []
        ...
        for x in range(SCROLL_RANGE):
            ...
            links = page.query_selector_all('div.cqKibH a')

            # Process the links
            for link in links:
                href = link.get_attribute('href')
                links_list.append(WEBSITE_URL + href)
